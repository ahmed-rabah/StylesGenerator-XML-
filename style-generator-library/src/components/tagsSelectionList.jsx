...
export default function DropdownMenu({ handleAdd }) {
    ...
    return (
        <div className="relative col-span-2 bg-white shadow-lg p-2 rounded-lg hidden md:block border-r max-h-[80vh] overflow-scroll hidden-scrollbar">
    ...
    return (
        <li key={category} >
            ...
            >
                ...
                {open ? <ChevronUp size={20} strokeWidth={2.5} className="transition-transform ease-in-out duration-150 hover:scale-110" /> : <ChevronDown size={20} className="transition-transform ease-in-out duration-150 hover:scale-110" strokeWidth={2.5} />}
            </button>
                    ...
                    <ul className="pt-2">
                        {
                            categoryItems.map((item, index) => (
                                <li key={index}
                                    className="px-2 py-1 hover:bg-gray-100 font-semibold cursor-pointer border-b-2 text-center"
