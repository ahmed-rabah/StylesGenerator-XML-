            ...
            <ul>
                {
                    props.theme.map(elem => (
                        <ThemeElement key={elem.id} elem={elem} handleDelete={props.handleDelete} handleEdit={props.handleEdit} selectedElem={props.selectedElem} />
                    ))
                }
            </ul>
        </div>
...
function ThemeElement({ elem, handleDelete, handleEdit, selectedElem }) {

    let title = names[elem.tag]
    return (
        <li key={elem.id}
            className={`border p-2 rounded-xl flex justify-between items-center mb-2 ${elem.id === selectedElem ? "bg-green-500" : ""}`}>
