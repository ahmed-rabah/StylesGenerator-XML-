...
function ThemeElement({ elem, handleDelete, handleEdit, selectedElement }) {
    ...
    return (
        <li key={elem.id}
            className={`border-2 p-2 rounded-xl flex justify-between items-center mb-2 ${selected ? "bg-sky-100 border-indigo-300" : ""}`}>
            <p className={` text-2xl ${selected ? "text-gray-700 font-semibold" : " text-emerald-600"}`}>
                {title} : <span className={`font-semibold ${selected ? " " : "text-blue-950"}`}>{` <${elem.tag}>`}</span>
            </p>
