...
function ThemeElement({ elem, handleDelete, handleEdit, selectedElement }) {
    ...
    return (
        <li key={elem.id}
            className={`border p-2 rounded-xl flex justify-between items-center mb-2 ${selected ? "bg-green-300 border-gray-200" : ""}`}>
            <p className={`text-2xl ${selected ? "text-" : "text-emerald-600"}`}>
