            ...
            <ul>
                {
                    props.theme.map(elem => (
                        <ThemeElement key={elem.id} elem={elem} handleDelete={props.handleDelete} handleEdit={props.handleEdit} selectedElem={props.selectedElement} />
                    ))
                }
            </ul>
        </div>
...
function ThemeElement({ elem, handleDelete, handleEdit, selectedElement }) {
    ...
    return (
        <li key={elem.id}
            className={`border p-2 rounded-xl flex justify-between items-center mb-2 ${elem.id === selectedElement ? "bg-green-500" : ""}`}>
